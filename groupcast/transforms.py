...
class SportSpecificTransform(torch.nn.Module):
    ...
    def get_sport_dimensions(self):
        ...
        if self.sport not in dimensions:
            raise ValueError(
                f"Sport '{self.sport}' not recognized or dimensions not defined."
            )
    ...
    def forward(self, players_coordinates: torch.Tensor) -> torch.Tensor:
        ...
        players_coordinates[:, :, 0] = torch.clamp(
            players_coordinates[:, :, 0], 0, width
        )
        players_coordinates[:, :, 1] = torch.clamp(
            players_coordinates[:, :, 1], 0, height
        )
...
class Resample(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x_resampled = F.interpolate(
            x, size=new_seq_len, mode="linear", align_corners=True
        )
